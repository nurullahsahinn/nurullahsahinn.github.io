var Input = {
  ...
  window.addEventListener('resize', function() {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    // Yeniden boyutlandırmadan sonra context ayarlarını tekrar uygula
    ctx.strokeStyle = "rgba(242, 72, 11, 0.25)";
    ctx.lineWidth = 3;
  });
  ...
  // Ekran boyutuna göre farklı parametrelerle animasyonu başlat
  if (window.innerWidth < 768) { // Mobil görünüm için sınır (ayarlanabilir)
    var legNumMobile = Math.floor(3 + Math.random() * 4); // 3-6 bacak
    setupLizard(
      4 / Math.sqrt(legNumMobile), // Daha küçük boyut
      legNumMobile,
      Math.floor(2 + Math.random() * legNumMobile * 4) // Daha kısa kuyruk
    );
  } else { // Masaüstü görünüm
    var legNumDesktop = Math.floor(1 + Math.random() * 12);
    setupLizard(
      8 / Math.sqrt(legNumDesktop),
      legNumDesktop,
      Math.floor(4 + Math.random() * legNumDesktop * 8)
    );
  }